...
from dqn_cnn_model import DQN_CNN_Model, DQN_CNN_Model_With_Attention
...
def load_dqn_agent(env,loadPath=None, buffer_size=BUFFER_SIZE, batch_size=BATCH_SIZE, learning_rate=LEARNING_RATE, gamma=GAMMA,
                    epsilon_i=EPSILON_INI, epsilon_f=EPSILON_MIN,epsilon_anneal_steps= EPSILON_ANNEAL_STEPS, episode_block=EPISODE_BLOCK, run_name="run", use_prioritized_replay=False, adaptive_epsilon=False):
  ...
  # obtener DQN AGENT
    dqn_agent = DQNAgent(env, net, process_state, buffer_size, batch_size, learning_rate, gamma, epsilon_i, epsilon_f,epsilon_anneal_steps, episode_block, device=DEVICE, run_name=run_name, use_prioritized_replay=use_prioritized_replay, adaptive_epsilon=adaptive_epsilon)
    return dqn_agent
...
def execute_dqn_training_phase(phase_id, reference_states, loadPath = None, total_steps = TOTAL_STEPS, episodes = EPISODES, epsilon_i = EPSILON_INI, epsilon_f = EPSILON_MIN, epsilon_anneal_steps = EPSILON_ANNEAL_STEPS, gamma=GAMMA, use_prioritized_replay=False, adaptive_epsilon=False):
    video_folder = f'./videos/dqn/{phase_id}'
    env = create_env(video_folder=video_folder)
    dqn_agent = load_dqn_agent(env, loadPath=loadPath, epsilon_i=epsilon_i, epsilon_f=epsilon_f, epsilon_anneal_steps=epsilon_anneal_steps, episode_block=EPISODE_BLOCK, run_name=phase_id, gamma=gamma, use_prioritized_replay=use_prioritized_replay,  adaptive_epsilon=adaptive_epsilon)
...
def execute_ddqn_training_phase(phase_id, reference_states, load_net_path = None, total_steps = TOTAL_STEPS, episodes = EPISODES, epsilon_ini = EPSILON_INI, epsilon_min = EPSILON_MIN, epsilon_anneal_steps = EPSILON_ANNEAL_STEPS, gamma=GAMMA, learning_rate=LEARNING_RATE, use_attention=False):
    print("Parametros del agente:")
    ...
    modelo_a = DQN_CNN_Model_With_Attention(env.observation_space.shape, env.action_space.n).to(DEVICE) if use_attention else DQN_CNN_Model(env.observation_space.shape, env.action_space.n).to(DEVICE)
    if(load_net_path is not None):
        modelo_a.load_state_dict(torch.load(load_net_path))
    modelo_b = DQN_CNN_Model_With_Attention(env.observation_space.shape, env.action_space.n).to(DEVICE) if use_attention else DQN_CNN_Model(env.observation_space.shape, env.action_space.n).to(DEVICE)
...
def getVideoFolder(phase_id, type=DQN_TYPE):
    return f'./videos/{type}/validation/{phase_id}'


def getVideoPath(phase_id, type=DQN_TYPE):
    VALIDATION_VIDEO_FOLDER = getVideoFolder(phase_id, type=type)
    return f"{VALIDATION_VIDEO_FOLDER}/breakout-episode-0.mp4"
