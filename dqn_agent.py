...
class DQNAgent(Agent):
    def __init__(self, env, model, obs_processing_func, memory_buffer_size, batch_size, learning_rate, gamma, epsilon_i, epsilon_f, epsilon_anneal_steps, episode_block, device):
        ...
        # Movemos la red al dispositivo (CPU/GPU) para cómputo eficiente de tensores
        self.policy_net = model.to(device)

        # Configurar función de pérdida MSE y optimizador Adam
        # MSELoss mide la diferencia cuadrática entre predicción y target, estable para Q-learning
        # Adam ajusta dinámicamente las tasas de aprendizaje de cada parámetro
        ...
        # Batch_size define cuántas muestras usar en cada actualización
        self.batch_size = batch_size

        # Gamma es el factor de descuento para valorar recompensas futuras
        self.gamma = gamma
        
        # Parámetros de epsilon-greedy determinan exploración vs explotación
